...
def generate_eval_set(num_questions: int) -> list[dict]:
    ...
    session_store = {k: v for k, v in store.DOCUMENT_STORE.items() if k in store.SESSION_DOCS}
    ...
    logger.info(
        "Eval-gen complete: %d questions across %d doc(s)",
        len(store.EVAL_SET),
        len(store.DOCUMENT_STORE),
    )
...
def run_evaluation(k: int) -> dict:
    ...
    for item in store.EVAL_SET:
        ...
        f_hit = int(correct in f_res)
        r_hit = int(correct in r_res)
        h_hit = int(correct in h_res)
